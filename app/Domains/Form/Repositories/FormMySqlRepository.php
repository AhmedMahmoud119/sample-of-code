...
class FormMySqlRepository implements FormRepositoryInterface
{
    ...
    {
        return $this->form::with(['formVersions' => function ($query) {
            $query->orderBy('updated_at', 'desc')->select(['form_id', 'updated_at as last_update']);
        }])->select(['id', 'title'])->withCount('formVersions')
            ->get();
    }
    ...
    public function viewResponses($id, $request)
    {
        ...
        $fieldResponse = FieldResponse::whereIn('submission_id', $fieldResponseQuery->pluck('submission_id')->toArray())
            ->whereHas('formField',function($q){
                $q->where('hidden',0);
            })
            ->get()
            ...
            },SORT_REGULAR,request()->sort_type == 'desc'?true:false)
    ...
    {
        $formVersion = FormVersion::with(['fields.field',
            'fields' => function($q){
                $q->where('hidden',0);
            }
    ...
    public function deleteFormField($form_field_id)
    {
        ...
        if ($formField->has('fieldResponses')) {
            return false;
        }
        $formField->delete();
    ...
    public function addField($id, $request)
    {
        ...
        try {
            ...
            } elseif (count($lastVersion->fields->pluck('fieldResponses')->flatten()->toArray()) == 0) {
    ...
    public function getGroupedResponses($submissionIds){
        return FieldResponse::with('formField')
            ->whereIn('submission_id', $submissionIds)
            ->get()->sortBy(function($response) {
